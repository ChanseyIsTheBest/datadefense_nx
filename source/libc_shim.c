...
#include "config.h"
/* Unconditional on purpose. The gate controls whether a pack is ever BUILT or
 * MOUNTED, not whether these symbols exist -- asset_pack.c is compiled by the
 * Makefile wildcard either way. Five call sites below (opendir/closedir/
 * readdir/fopen/pread) were never wrapped in #if KB_ASSET_PACK, so guarding
 * the include meant KB_ASSET_PACK=0 failed to compile. It had simply never
 * been tried: Killer Bean shipped with the packer on. Every one of those
 * functions returns "not mine" when no pack is mounted, so leaving them
 * callable is correct and costs nothing. */
...
// (android_native_unity.c). Real files (small fds from open()) pass through to newlib.
// ---------------------------------------------------------------------------
...
/* dlopen: succeed for the libraries we actually shim, FAIL for the ones we
 * deliberately do not provide.
 *
 * This used to return a non-NULL handle for EVERY name, which is worse than it
 * looks. A caller that gets a handle proceeds to dlsym() the entry points it
 * needs; our dlsym returns NULL for a library we have no shim for; and the
 * caller then either takes a confused half-initialised path or calls through a
 * NULL function pointer. Returning NULL from dlopen instead makes the caller
 * take the ONE path it has actually tested: "this library is not present".
 *
 * Two of this game's subsystems are configured entirely through this list,
 * which is why it exists rather than a set of libunity offset patches:
 *
 *   libswappywrapper.so -- Unity 6 loads Swappy through a wrapper .so and
 *     calls UnitySwappyWrapperInit() in it. Killer Bean had to patch libunity
 *     to force frame pacing off, because on that engine Swappy was compiled in
 *     and its init hung waiting for Choreographer callbacks. Here the wrapper
 *     is a separate library that IS NOT SHIPPED IN THE APK AT ALL -- the arm64
 *     folder holds exactly libmain, libunity and libil2cpp. So Swappy cannot
 *     initialise no matter what we do, and the engine already has a clean,
 *     logged failure path for it ("Failed to load libswappywrapper.so").
 *     Denying it here just makes that outcome deliberate instead of accidental.
 *
 *   libaaudio.so -- Unity 6's FMOD ships two Android output backends, AAudio
 *     and OpenSL (fmod_output_aaudio.cpp / fmod_output_opensl.cpp). Note what
 *     is NOT there: the Java AudioTrack output that Killer Bean had to patch
 *     libunity to steer away from is gone in this engine, so that patch has no
 *     purpose here. What we want is for FMOD to skip AAudio, which we have no
 *     implementation for, and use OpenSL, which opensles.c implements. Failing
 *     this dlopen is exactly how a device without AAudio behaves, so FMOD's own
 *     fallback does the work.
 *
 * Unknown names DENY rather than allow. A library we have not thought about is
 * one we have no shim for, and the failure we want in that case is the honest
 * one.
 */
static const char *const dlopen_allow[] = {
  "libGLESv2.so", "libGLESv3.so", "libEGL.so", "libOpenSLES.so",
  /* libaaudio.so is now ALLOWED, and that is a reversal worth explaining.
   * Denying it was right while we believed FMOD would fall back to OpenSL --
   * "a device without AAudio" was exactly the behaviour we wanted. But this
   * build of FMOD has no OpenSL output plugin at all (only fmod_output_aaudio
   * and fmod_output_audiotrack are compiled in), so the denial did not select
   * a fallback, it selected FMOD_ERR_OUTPUT_INIT and silence. aaudio_shim.c
   * implements the API over audout; dlsym_fake resolves the symbols from the
   * import table. */
  "libaaudio.so",
  "libandroid.so", "liblog.so", "libc.so", "libdl.so", "libm.so",
  "libz.so", "libstdc++.so", "libc++_shared.so",
};

static const char *dlopen_basename(const char *p) {
...
void *dlopen_fake(const char *name, int flags) {
  (void)flags;
  if (!name) { debugPrintf("dlopen(self) -> handle\n"); return (void *)0x1; }
  const char *base = dlopen_basename(name);
  for (unsigned i = 0; i < sizeof(dlopen_allow) / sizeof(*dlopen_allow); i++) {
    if (strcmp(base, dlopen_allow[i]) == 0) {
      debugPrintf("dlopen(%s) -> handle [shimmed]\n", name);
      return (void *)0x1;
    }
  }
  /* A module we loaded ourselves (libmain/libunity/libil2cpp) is also fine. */
  if (so_resolve_external_module(base)) {
    debugPrintf("dlopen(%s) -> handle [loaded module]\n", name);
    return (void *)0x1;
  }
  debugPrintf("dlopen(%s) -> NULL [not provided; caller takes its "
              "library-absent path]\n", name);
  return NULL;
}
 ...
 * eight globals in nx_patch_datadefense.h came out of THIS libil2cpp.so by
 ...
 * table below; this port scans all 256 buckets rather than reproducing the
 * hash, so there is no hash constant to get wrong.
 *
...
#include "nx_patch_datadefense.h"   /* GC_*_OFF_FN, derived for this game */
...
/* Is the world stopped right now, and who stopped it?
 *
 * A brand-new thread must not run engine code while a stop-the-world is in
 * progress. It is not in GC_threads yet -- Boehm registers a thread from
 * inside its own entry -- so the collector neither pauses it nor knows to wait
 * for it. If it then takes a lock held by a thread the collector HAS paused,
 * it blocks until the world restarts, and anything waiting on that new thread
 * blocks with it.
 *
 * That is the intermittent boot hang: UnityMain parked in
 * PlatformThread::Create's `ldr w8,[x20,#0x20] / cond_wait / retry` loop
 * waiting for its new thread to publish "I am up", while the new thread sat on
 * a futex whose owner was suspended. The window is the few microseconds
 * between pthread_create returning and the new thread registering, which is
 * why it survived one boot and not the next.
 *
 * Exposed rather than inferred: the trampoline needs the real flag, and a
 * second copy of this state would be one more thing to keep in step. */
int nx_gc_world_stopped(void) { return g_gc_stopped; }
 ...
 * derived from THIS libil2cpp (see nx_patch_datadefense.h) and are checked by
...
/* Finding a thread's GC record.
 *
 * Killer Bean reproduced GC_lookup_thread's hash (lsr/eor/eor/and, read off
 * ITS binary at 0x160ea98) so the lookup could go straight to the right
 * bucket. That constant is binary-specific and was never re-derived for Data
 * Defense, and a wrong hash here does not fail loudly -- it silently returns
 * NULL for threads that do exist, so the GC quietly stops capturing them.
 *
 * The table is 256 buckets of chained records (cmp x21,#0x100 in both
 * GC_suspend_all and GC_restart_all, which walk it exactly this way). Scanning
 * all of them removes the constant entirely. It runs once per thread per GC
 * stop over a handful of live threads, so the cost is nil, and it cannot be
 * wrong the way a mis-derived hash can.
 */
static NxGcThread *nx_gc_find_thread(pthread_t id) {
  ...
  for (unsigned b = 0; b < 256; b++) {
    for (NxGcThread *t = table[b]; t; t = t->next)
      if (t->id == id) return t;
  }
  return NULL;
}
