 ...
 * real OS fd table. Ported from cr3_nx's android_native_unity.c but standalone: Unity
...
/* Bytes available on a pipe read end, without consuming them.
 *
 * Needed by ALooper_pollOnce: Unity 6's NdkLooper delivers messages by writing
 * one byte to a pipe and relying on the looper to notice the fd is readable and
 * run the registered callback. Without a way to ask "is there anything here?"
 * the looper cannot dispatch, and the message never arrives. */
int fakefd_readable(int fd) {
  fakefd_once();
  int i = fd - FAKE_FD_BASE;
  if (i < 0 || i >= MAX_FAKE_FDS) return 0;
  mutexLock(&g_lock);
  int r = 0;
  if (g_fds[i].kind == FD_PIPE_R && g_fds[i].pipe)
    r = (g_fds[i].pipe->len > 0);
  mutexUnlock(&g_lock);
  return r;
}
