/* jni_fake.c -- fake JNI environment for the Unity engine.
 *
 * The header below described the MVGL engine (libcrx.so) from the Chaos Rings
 * lineage and was three engines out of date. What this file actually does here
 * is answer the JNI calls Unity's own Java layer would have served: it provides
 * a working JNIEnv/JavaVM so libunity and libil2cpp resolve their
 * FindClass/GetMethodID/Call*Method traffic to native code.
 *
 * For Data Defense this is the whole JNI story, because the game ships NO
 * custom Java classes -- only generated R classes. Every class named at runtime
 * is standard Unity/Android engine surface, which is exactly what this file and
 * unity_jni.c already cover.
 *
 * Historical note, kept because the structure still reflects it: originally for
 * the MVGL engine (libcrx.so)
 *
...
#include <stdlib.h>
#include <stdio.h>
#include <sys/stat.h>   /* stat(): findLibrary reports whether the .so is really there */
#include <stdint.h>
#include <stdarg.h>
#include <string.h>
#include <pthread.h>
...
/* Receiver-label routing. --------------------------------------------------
 *
 * Unity 6 hands us "java/lang/Object" for id->cls on almost every call: 151 of
 * the 153 JNI ids resolved in this game's boot, 99%. So every
 * name_has(id->cls, "SomeClass") gate below is dead code for practically all
 * traffic, and the miss is silent -- the call falls through to a default and
 * returns 0 or NULL.
 *
 * The receiver survives, though:
 *   - instance calls: recv is whatever we returned from
 *     jni_make_object("android/os/Bundle") etc, interned by label;
 *   - static calls:   recv is the jclass, i.e. our own FakeClass, whose name is
 *     the class the game actually asked FindClass for.
 *
 * fake_class_name() reads both forms. cls_or_recv() is a drop-in for a bare
 * class gate and keeps the class test FIRST, so nothing changes on a call that
 * does arrive with a real class.
 *
 * Deliberately NOT a name-only match: method names like getString / getInstance
 * / get are ambiguous across classes, whereas the label is not. (Field
 * CONSTANTS are different -- SDK_INT names exactly one field in the whole
 * Android API -- which is why field_int answers those by name.)
 *
 * Adopted from hitmansniper_nx, which measured the same 90% collapse and built
 * this. */
/* name_has() is defined further down; these helpers sit up here because the
 * dispatchers below need them. hitmansniper_nx carries the same two forward
 * declarations for the same reason -- I dropped them on the way across and the
 * file stopped compiling. */
static int name_has(const char *name, const char *sub);

static const char *fake_class_name(void *jcls) {
  if (!jcls) return NULL;
  uint32_t tag = nx_tag_of(jcls);
  if (tag == TAG_CLASS)  return ((FakeClass  *)jcls)->name;
  if (tag == TAG_OBJECT) return ((FakeObject *)jcls)->label;
  return NULL;
}
static int recv_is(void *recv, const char *label) {
  if (!recv) return 0;
  const char *l = fake_class_name(recv);
  return l && l[0] && strstr(l, label) != NULL;
}
static int cls_or_recv(const FakeID *id, void *recv, const char *what) {
  return (id && name_has(id->cls, what)) || recv_is(recv, what);
}
...
static void *dispatch_object(void *recv, const FakeID *id, va_list va) {
  /* OBJECT-returning calls on one of our synthetic input events.
   *
   * The [in.i] and [in.f] traces cover int and float getters. Nothing covered
   * object getters, and getDevice() is one -- so a whole category of calls was
   * invisible. That mattered: AndroidCursors' IsInputDeviceValid() calls
   * event.getDevice() and rejects the event if it comes back null, and the
   * absence of getDevice from the trace was read as "never called" when it
   * could equally have meant "never traced".
   *
   * Not assuming that again. */
  if (input_owns_recv(recv)) {
    static unsigned n;
    if (n < 40) { n++;
      debugPrintf("    [in.o] %s  (cls=%s) -> object getter on OUR event\n",
                  id->name, id->cls); }
  }
  ...
  if (recv && cls_or_recv(id, recv, "$Builder")) {
    const char *rp = strchr(id->sig, ')');
  ...
  /* ROUTED BY SIGNATURE, NOT CLASS. This was gated on
   * input_owns_class(id->cls), and the class does not survive: the log shows
   * the id arriving as
   *
   *     JNI id: UnityPlayer.obtain (Landroid/view/MotionEvent;)Landroid/view/MotionEvent;
   *
   * so the gate was false, obtain fell through to the generic path, and the
   * engine got a junk copy. It then stopped -- resolving getSource and never
   * getX, getY or getPointerCount.
   *
   * The int and float dispatchers survive the same collapse because they test
   * input_owns_recv(recv) first. obtain cannot: it is STATIC, so there is no
   * receiver and the event is the ARGUMENT. The signature is the only reliable
   * signal, and it is a strong one -- exactly one method in the Android API
   * takes a MotionEvent and returns a MotionEvent. */
  if (!strcmp(id->name, "obtain") &&
      strstr(id->sig, "(Landroid/view/MotionEvent;)Landroid/view/MotionEvent;")) {
    void *src = va_arg(va, void *);
    void *cp  = unity_motionevent_obtain(src);
    static unsigned n;
    if (n < 6) { n++;
      debugPrintf("    [in.o] MotionEvent.obtain(%p) -> %p  (cls=%s, routed by "
                  "signature)\n", src, cp, id->cls); }
    return cp;
  }
  ...
  if (recv && nx_tag_of(recv) == TAG_OBJECT &&
      cls_or_recv(id, recv, "$Builder") && sig_returns(id->sig, "$Builder;"))
    return recv;
   ...
   * nobody can see. */
  if (recv && cls_or_recv(id, recv, "AlertDialog$Builder") &&
  ...
  if (recv && nx_tag_of(recv) == TAG_OBJECT &&
      (cls_or_recv(id, recv, "java/lang/StringBuilder") ||
       cls_or_recv(id, recv, "java/lang/StringBuffer"))) {
  ...
  if (recv &&
      (cls_or_recv(id, recv, "java/util/Scanner") ||
       cls_or_recv(id, recv, "java/lang/StringBuilder") ||
       cls_or_recv(id, recv, "java/lang/StringBuffer") ||
       cls_or_recv(id, recv, "AlertDialog$Builder"))) {
    char want[160];
  ...
  /* ClassLoader.findLibrary(String)Ljava/lang/String; -- UNITY 6 NEEDS THIS.
   *
   * The comment on "loadLibrary" further up says findLibrary "is never
   * consulted". That was true of Unity 2021.3. Unity 6 consults it
   * unconditionally: UnityInitApplication does
   *
   *     DVM::FindLibrary("il2cpp", &path)   -> ClassLoader.findLibrary("il2cpp")
   *     LoadIl2Cpp(path)                    -> dlopen(path) + dlsym(il2cpp_init)
   *
   * and if either step fails it calls
   *
   *     android::ui::DisplayDialog("Error", "Failed to load Il2CPP.")
   *
   * which blocks on a condvar waiting for a dialog dismissal that can never
   * arrive here. That is a black screen with a live, spinning main thread and
   * no error text anywhere -- the hang looks like a job-system deadlock and is
   * nothing of the kind.
   *
   * The loader has already mapped libil2cpp.so, so all that is needed is a
   * path whose BASENAME matches: dlopen_fake resolves already-loaded modules
   * through so_resolve_external_module(), and dlsym_fake then finds il2cpp_init
   * as a real export. Returning the actual on-card path keeps it honest and
   * makes the log readable if the load still fails.
   *
   * Android's findLibrary takes the bare name ("il2cpp") and returns a full
   * path, so the "lib" prefix and ".so" suffix are ours to add. */
  if (!strcmp(id->name, "findLibrary") &&
      sig_returns(id->sig, "Ljava/lang/String;")) {
    const char *want = jni_string_utf(va_arg(va, void *));
    char path[768];
    if (want && *want) {
      /* already fully decorated? ("libil2cpp.so" as well as "il2cpp") */
      int has_lib = !strncmp(want, "lib", 3);
      int has_so  = strlen(want) > 3 && !strcmp(want + strlen(want) - 3, ".so");
      snprintf(path, sizeof path, "%s/%s%s%s", DATA_ROOT,
               has_lib ? "" : "lib", want, has_so ? "" : ".so");
    } else {
      path[0] = '\0';
    }
    struct stat st;
    int there = path[0] && stat(path, &st) == 0;
    debugPrintf("[jni] ClassLoader.findLibrary(\"%s\") -> \"%s\"%s\n",
                want ? want : "(null)", path,
                there ? "" : "   *** NOT ON THE CARD ***");
    /* Hand the path back even when the file is missing: Unity's own
     * "Unable to find library path" / dlopen failure is a better diagnostic
     * than an empty string, which it reports as a bare findLibrary failure. */
  ...
  /* Any method returning an android.graphics.Rect -- getBounds() is the one
   * this game calls (`JNI id: java/lang/Object.getBounds ()Landroid/graphics/
   * Rect;` in the log). Routed by RETURN TYPE rather than class name for the
   * same reason as Bitmap below: the class reads back as java/lang/Object.
   *
   * Returning NULL here is what the generic path did, and Unity dereferences
   * the result to size the window. A labelled Rect plus real left/top/right/
   * bottom (see field_int) reports the actual surface instead. */
  if (sig_returns(id->sig, "Landroid/graphics/Rect;")) {
    debugPrintf("[jni] %s -> Rect(0,0,%d,%d)\n",
                id->name, screen_width, screen_height);
    return jni_make_object("android/graphics/Rect");
  }
...
static void dispatch_void(void *recv, const FakeID *id, va_list va) {
  ...
  jni_approx_arm();
  log_app_upcall(id);
  if (cls_or_recv(id, recv, "FMODAudioDevice")) {
...
static void *j_GetObjectClass(void *env, void *obj) {
  ...
  /* Synthetic input events know their own concrete class, and the engine needs
   * it. Handing back java/lang/Object for a MotionEvent leaves nativeInjectEvent
   * unable to tell motion from key -- and it guesses KEY. The r34 log is
   * unambiguous: the engine asked getKeyCode, getAction, getMetaState,
   * getDeviceId and getRepeatCount (a KeyEvent-only method, 24 times) and never
   * once asked getX, getY or getPointerCount. It had committed to the wrong
   * type before it ever looked at the coordinates.
   *
   * This is the same class-collapse that broke SDK_INT and the KEYCODE
   * constants, in its third form: there the fake could not identify a class and
   * answered 0, here it cannot identify a class and answers java/lang/Object.
   * Where the object DOES know what it is, say so. */
  { const char *cn = unity_event_class(obj);
    if (cn) {
      static const char *seen; if (seen != cn) { seen = cn;
        debugPrintf("[jni] GetObjectClass -> %s (was java/lang/Object)\n", cn); }
      return intern_class(cn);
    } }
  /* An object we made with a label knows its class too -- jni_make_object
   * interns by label, and that label IS the Java class name the caller asked
   * for. Returning it costs nothing and makes every class-gated rule in this
   * file reachable through the GetObjectClass path, which 6k measured at 99%
   * collapsed. */
  { const char *lbl = fake_class_name(obj);
    if (lbl && lbl[0] && strchr(lbl, '/')) return intern_class(lbl); }
  return intern_class("java/lang/Object");
}
...
static juint j_IsInstanceOf(void *env, void *obj, void *clazz) {
  if (input_owns_recv(obj))
    debugPrintf("    [in.?] IsInstanceOf(OUR event, %s)\n",
                class_name_of(clazz));
  (void)env;
  ...
  if (input_owns_recv(obj)) {
    ...
    /* The class could not be named. `return 1` here was correct for the
     * InputEvent BASE class and fatal for an unknown one -- see
     * input_begin_dispatch() in unity_input.c. nativeInjectEvent runs exactly
     * two tests per event, KEY first then MOTION, so answer by position. */
    { int nth = input_instanceof_next();
      int motion = input_recv_is_motion(obj);
      int ans = (nth == 0) ? (motion ? 0 : 1)    /* test 0 asks: is it a KEY?   */
                           : (motion ? 1 : 0);   /* test 1 asks: is it MOTION?  */
      static unsigned n;
      if (n < 12) { n++;
        debugPrintf("    [in.?] instanceof #%d on a %s event -> %d "
                    "(class unnamed)\n", nth, motion ? "MOTION" : "KEY", ans); }
      return (juint)ans; }
  }
...
static juint field_int(const FakeID *id) {
  ...
  /* CLASS-INDEPENDENT FALLBACKS.
   *
   * Everything above is gated on the class name, which fails whenever this
   * fake collapses a class it does not recognise to java/lang/Object -- and
   * the field ID then carries "java/lang/Object" rather than "android/os/
   * Build$VERSION". SDK_INT was read exactly that way:
   *
   *     JNI id: java/lang/Object.SDK_INT I
   *
   * so `name_has(c, "os/Build")` was false, field_int fell through to 0, and
   * the engine reported `OS 'Android OS (API 0)'`.
   *
   * That is not cosmetic. AndroidGraphics gates the NDK Choreographer on
   * `apiLevel > 23` (libunity+0xa99f9c); at 0 it logs "NDK Choreographer
   * requires device API Level 24 ... Fallbacking to Java Choreographer." and
   * takes a path that waits on a condvar for a Java callback that cannot exist
   * here. The result is the render loop stopping dead after two frames with
   * every worker parked -- with no error, because from Unity's point of view
   * nothing went wrong.
   *
   * These names are unambiguous across the whole Android API, so matching on
   * the name alone is safe. Anything class-specific stays above. */
  if (!strcmp(n, "SDK_INT"))         return NX_SDK_INT;
  if (!strcmp(n, "PREVIEW_SDK_INT")) return 0;

  /* The same class-collapse defeats seven more integer constants that the
   * engine reads through java/lang/Object in this build. All are fixed values
   * in the Android SDK -- there is no device on which they differ -- so
   * answering by name costs nothing and removes a whole row of silent zeros.
   *
   * SCREEN_ORIENTATION_* matter here: this game is landscape, and a 0 for
   * LANDSCAPE is indistinguishable from a 0 for UNSPECIFIED. */
  if (!strcmp(n, "SCREEN_ORIENTATION_LANDSCAPE"))          return 0;   /* ActivityInfo */
  if (!strcmp(n, "SCREEN_ORIENTATION_PORTRAIT"))           return 1;
  if (!strcmp(n, "SCREEN_ORIENTATION_REVERSE_LANDSCAPE"))  return 8;
  if (!strcmp(n, "SCREEN_ORIENTATION_REVERSE_PORTRAIT"))   return 9;
  if (!strcmp(n, "SCREEN_ORIENTATION_SENSOR_LANDSCAPE"))   return 6;
  if (!strcmp(n, "SCREEN_ORIENTATION_FULL_SENSOR"))        return 10;
  if (!strcmp(n, "SCREEN_ORIENTATION_FULL_USER"))          return 13;
  if (!strcmp(n, "PERMISSION_GRANTED"))                    return 0;   /* PackageManager */
  if (!strcmp(n, "GET_DEVICES_OUTPUTS"))                   return 2;   /* AudioManager  */
  if (!strcmp(n, "ROUTE_TYPE_LIVE_VIDEO"))                 return 2;   /* MediaRouter   */

  /* android.view.KeyEvent key codes. THIS IS WHY TOUCH DID NOT WORK.
   *
   * nativeInjectEvent()'s first move on any InputEvent is getKeyCode(), and it
   * compares the result against KEYCODE_VOLUME_DOWN before deciding the event
   * is a motion. A MotionEvent's keycode field is 0 (KEYCODE_UNKNOWN, never
   * set), and KEYCODE_VOLUME_DOWN was falling through to 0 as well -- so the
   * comparison was 0 == 0 and the engine swallowed EVERY touch as a volume-key
   * press:
   *
   *     [jni] int field -> 0 (unmatched): java/lang/Object.KEYCODE_VOLUME_DOWN
   *     [touch] inject(action=0x0, n=1) frame=72 -> DROPPED
   *
   * Two independently sufficient bugs met: an unanswered constant, and a
   * default that happened to collide with it. The unmatched-field diagnostic
   * added in 6k is what made it visible -- without that line this looks like
   * "the engine ignores touches".
   *
   * Values are the Android SDK's and are fixed across every device. */
  if (!strcmp(n, "KEYCODE_UNKNOWN"))          return 0;
  if (!strcmp(n, "KEYCODE_HOME"))             return 3;
  if (!strcmp(n, "KEYCODE_BACK"))             return 4;
  if (!strcmp(n, "KEYCODE_DPAD_UP"))          return 19;
  if (!strcmp(n, "KEYCODE_DPAD_DOWN"))        return 20;
  if (!strcmp(n, "KEYCODE_DPAD_LEFT"))        return 21;
  if (!strcmp(n, "KEYCODE_DPAD_RIGHT"))       return 22;
  if (!strcmp(n, "KEYCODE_DPAD_CENTER"))      return 23;
  if (!strcmp(n, "KEYCODE_VOLUME_UP"))        return 24;
  if (!strcmp(n, "KEYCODE_VOLUME_DOWN"))      return 25;
  if (!strcmp(n, "KEYCODE_POWER"))            return 26;
  if (!strcmp(n, "KEYCODE_ENTER"))            return 66;
  if (!strcmp(n, "KEYCODE_DEL"))              return 67;
  if (!strcmp(n, "KEYCODE_MENU"))             return 82;
  if (!strcmp(n, "KEYCODE_SEARCH"))           return 84;
  if (!strcmp(n, "KEYCODE_MEDIA_PLAY_PAUSE")) return 85;
  if (!strcmp(n, "KEYCODE_MEDIA_STOP"))       return 86;
  if (!strcmp(n, "KEYCODE_MEDIA_NEXT"))       return 87;
  if (!strcmp(n, "KEYCODE_MEDIA_PREVIOUS"))   return 88;
  if (!strcmp(n, "KEYCODE_VOLUME_MUTE"))      return 164;
  if (!strcmp(n, "KEYCODE_ESCAPE"))           return 111;
  if (!strcmp(n, "KEYCODE_BUTTON_A"))         return 96;
  if (!strcmp(n, "KEYCODE_BUTTON_B"))         return 97;
  if (!strcmp(n, "KEYCODE_BUTTON_X"))         return 99;
  if (!strcmp(n, "KEYCODE_BUTTON_Y"))         return 100;
  if (!strcmp(n, "KEYCODE_BUTTON_START"))     return 108;
  if (!strcmp(n, "KEYCODE_BUTTON_SELECT"))    return 109;
  if (!strcmp(n, "KEYCODE_ZOOM_IN"))          return 168;
  if (!strcmp(n, "KEYCODE_ZOOM_OUT"))         return 169;
  if (!strcmp(n, "KEYCODE_MEDIA_PLAY"))       return 126;
  if (!strcmp(n, "KEYCODE_MEDIA_PAUSE"))      return 127;
  if (!strcmp(n, "KEYCODE_HEADSETHOOK"))      return 79;
  if (!strcmp(n, "KEYCODE_CAMERA"))           return 27;
  if (!strcmp(n, "KEYCODE_TAB"))              return 61;
  if (!strcmp(n, "KEYCODE_SPACE"))            return 62;

  /* The other two constants the same diagnostic caught this boot. */
  if (!strcmp(n, "STREAM_MUSIC"))             return 3;    /* AudioManager */
  if (!strcmp(n, "uiMode"))                   return 0x11; /* NORMAL | NIGHT_NO */
  if (!strcmp(n, "SCREEN_ORIENTATION_UNSPECIFIED"))        return -1;  /* ActivityInfo */
  if (!strcmp(n, "SCREEN_ORIENTATION_USER"))               return 2;
  if (!strcmp(n, "SCREEN_ORIENTATION_SENSOR"))             return 4;
  if (!strcmp(n, "SCREEN_ORIENTATION_SENSOR_PORTRAIT"))    return 7;
  if (!strcmp(n, "SCREEN_ORIENTATION_USER_LANDSCAPE"))     return 11;
  if (!strcmp(n, "SCREEN_ORIENTATION_USER_PORTRAIT"))      return 12;
  if (!strcmp(n, "MODE_PRIVATE"))                          return 0;   /* Context */

  /* android.graphics.Rect public int fields. Reachable now that a Rect-returning
   * method hands back a real object rather than NULL. Left at 0 they report a
   * 0x0 window, which is a worse answer than the null-deref it replaces --
   * Unity sizes the screen from these. Match the geometry the surface actually
   * has, the same values force_geom() establishes. */
  if (!strcmp(n, "left") || !strcmp(n, "top"))  return 0;
  if (!strcmp(n, "right"))                      return (juint)screen_width;
  if (!strcmp(n, "bottom"))                     return (juint)screen_height;

  /* android.content.res.Configuration instance fields. The class-gated rule
   * further up never fires because the read arrives as java/lang/Object, and
   * two of these are not harmless zeros:
   *   orientation 0 is ORIENTATION_UNDEFINED, not LANDSCAPE (2)
   *   touchscreen 0 is TOUCHSCREEN_UNDEFINED, not FINGER (3) -- on a game
   *     whose entire input is touch
   * The dp sizes are derived from the real surface rather than invented, so
   * they stay consistent with what force_geom() establishes. */
  if (!strcmp(n, "orientation"))            return 2;    /* LANDSCAPE */
  if (!strcmp(n, "touchscreen"))            return 3;    /* FINGER    */
  if (!strcmp(n, "keyboard"))               return 1;    /* NOKEYS    */
  if (!strcmp(n, "keyboardHidden"))         return 2;    /* YES (no hw kbd) */
  if (!strcmp(n, "hardKeyboardHidden"))     return 2;    /* YES */
  if (!strcmp(n, "navigation"))             return 1;    /* NONAV */
  if (!strcmp(n, "navigationHidden"))       return 2;    /* YES */
  if (!strcmp(n, "screenLayout"))           return 0x24; /* SIZE_LARGE|LONG_NO */
  if (!strcmp(n, "colorMode"))              return 0x5;  /* WIDE_NO|HDR_NO */
  if (!strcmp(n, "fontWeightAdjustment"))   return 0;    /* UNDEFINED is 0 here */
  if (!strcmp(n, "mcc") || !strcmp(n, "mnc")) return 0;  /* no SIM: 0 is correct */
  if (!strcmp(n, "densityDpi"))             return 320;  /* xhdpi */
  if (!strcmp(n, "screenWidthDp"))          return (juint)(screen_width  * 160 / 320);
  if (!strcmp(n, "screenHeightDp"))         return (juint)(screen_height * 160 / 320);
  if (!strcmp(n, "smallestScreenWidthDp"))
    return (juint)((screen_width < screen_height ? screen_width : screen_height) * 160 / 320);

  /* ApplicationInfo SDK levels. 0 made Unity print
   * "Min API Level '0', Target API Level '0'"; anything gating on target SDK
   * would take its most conservative path. */
  if (!strcmp(n, "targetSdkVersion"))       return NX_SDK_INT;
  if (!strcmp(n, "minSdkVersion"))          return 24;

  /* Report unmatched integer fields once each.
   *
   * A silent 0 here is exactly how the SDK_INT bug hid for a whole session:
   * the read SUCCEEDS, the value is wrong, and nothing says so. There is no
   * failure to notice -- Unity took its pre-API-24 path, built the Java
   * Choreographer and parked forever, and the log looked clean.
   *
   * Adopted from hitmansniper_nx, which hit the identical bug and added this
   * afterwards. If some later failure smells like a wrong constant, this is
   * the first place to look. */
  {
    static char seen[24][48];
    static int n_seen;
    int dup = 0;
    for (int i = 0; i < n_seen; i++)
      if (!strcmp(seen[i], n)) { dup = 1; break; }
    if (!dup && n_seen < (int)(sizeof seen / sizeof seen[0])) {
      snprintf(seen[n_seen++], sizeof seen[0], "%s", n);
      debugPrintf("[jni] int field -> 0 (unmatched): %s.%s\n",
                  c && c[0] ? c : "?", n);
    }
  }
  return 0;
...
/* How many arguments does a JNI descriptor declare, plus the implicit
 * JNIEnv* and jclass/jobject that every native receives? */
static int nx_jni_argc(const char *sig) {
  if (!sig || *sig != '(') return -1;
  int n = 0;
  for (const char *p = sig + 1; *p && *p != ')'; p++) {
    if (*p == '[') continue;                 /* array marker, not an argument */
    if (*p == 'L') { while (*p && *p != ';') p++; }
    n++;
  }
  return n + 2;                              /* + JNIEnv*, + jclass/jobject */
}

/* Entry points main.c calls directly, with the argument count its typedef
 * uses. Keep in step with unity_entrypoints.h. */
static const struct { const char *name; int argc; } NX_ENTRYPOINTS[] = {
  { "initJni",                       5 },    /* env, thiz, Context, int, String */
  { "nativeRecreateGfxState",        4 },
  { "nativeSendSurfaceChangedEvent", 2 },
  { "nativeRender",                  2 },
  { "nativeInjectEvent",             4 },
  { "nativePause",                   2 },
  { "nativeResume",                  2 },
  { "nativeFocusChanged",            3 },
  { "nativeDone",                    2 },
  { "nativeApplicationUnload",       2 },
  { "nativeUnitySendMessage",        5 },
  { "nativeMuteMasterAudio",         3 },
  { "nativeIsAutorotationOn",        2 },
  { "nativeSetLaunchURL",            3 },
};

static void nx_check_entrypoint(const char *cls, const char *name,
                                const char *sig, void *fn) {
  for (unsigned k = 0; k < sizeof NX_ENTRYPOINTS / sizeof NX_ENTRYPOINTS[0]; k++) {
    if (strcmp(name, NX_ENTRYPOINTS[k].name)) continue;
    int real = nx_jni_argc(sig);
    if (real < 0) return;
    if (real != NX_ENTRYPOINTS[k].argc)
      debugPrintf("[entry] *** ARITY MISMATCH *** %s.%s %s takes %d args, "
                  "the loader calls it with %d. Surplus/missing arguments are "
                  "silent on AArch64 -- fix unity_entrypoints.h before trusting "
                  "anything after this line.\n",
                  cls, name, sig, real, NX_ENTRYPOINTS[k].argc);
    else
      debugPrintf("[entry] ok %s %s (%d args) -> %p\n", name, sig, real, fn);
    return;
  }
}
...
static juint j_RegisterNatives(void *env, void *cls, void *methods, int n) {
  ...
  if (m) {   /* dump the whole table once per class: the keyboard callback is in here */
    ...
    /* Cross-check the entry points the loader calls by hardcoded offset
     * against what the engine ACTUALLY registers.
     *
     * This exists because of the first boot on this game. The loader called
     * initJni through a three-argument typedef inherited from Unity 2021.3,
     * while Unity 6 declares it (Context, int, String). AArch64 passes surplus
     * arguments in registers and reads missing ones from whatever was already
     * there, so the wrong call produced no diagnostic at all -- Unity read a
     * random int as its context type and recursed into its error path until
     * the main thread's 1 MB stack was gone.
     *
     * The information needed to catch that was sitting right here the whole
     * time: the engine hands us the real signature string. Comparing the
     * argument count against the typedef costs one strchr and turns a stack
     * overflow eight thousand frames deep into one line at boot.
     *
     * Its own loop, deliberately. The dump above stops after 200 entries
     * globally (`dumped`) to keep debug.log readable, and an arity check that
     * inherited that cap would go quiet exactly when a big registration run
     * pushed the interesting class past the limit. This one runs over every
     * entry of every class, always. */
    for (int i = 0; i < n; i++)
      if (m[i].name && m[i].sig && m[i].fn)
        nx_check_entrypoint(cn, m[i].name, m[i].sig, m[i].fn);
  }
...
void jni_init(void) {
  ...
  /* GetStringCritical / ReleaseStringCritical. Same UTF-16 contract as
   * GetStringChars/ReleaseStringChars at 165/166 -- same signatures, and
   * returning a copy is a legal implementation of both -- so wire them to the
   * same code. Some marshalling routines take the Critical path instead, and
   * an unfilled slot here is the UNIMPL stub: it returns NULL and the caller
   * reads an empty string with no error anywhere.
   * Adopted from hitmansniper_nx; these were the only two env slots any port
   * fills that this tree did not. */
