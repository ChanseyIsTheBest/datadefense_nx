/* imports_ndk_stubs.c -- NDK entry points the engine links against that the
 * Switch has no equivalent for.
 *
 * RENAMED from imports_killerbean_extra.c, because that name was actively
 * wrong here. An audit of the symbols (tools/audit_sources.py) shows Data
 * Defense's libunity/libil2cpp import 67 of the names in this table -- it is
 * one of the most load-bearing files in the tree, not a leftover from another
 * port. Nothing about its contents is Killer-Bean-specific: it is the MediaNDK
 * / AImageReader / AHardwareBuffer surface that any Unity Android build links
 * against whether or not it uses those features.
 *
 * The stubs stay stubs. Verified against THIS game's managed code:
 *
 *   MediaNDK (AMediaCodec/Extractor/Format + the key strings) backs Unity's
 *   VideoPlayer hardware decode path. Data Defense's libil2cpp contains ZERO
 *   references to VideoPlayer, so these are link-satisfying only and can never
 *   be reached. They still fail fast and cleanly rather than blocking, so if
 *   that assumption is ever wrong the engine raises a prepare error instead of
 *   hanging.
 *
 *   AImageReader / AHardwareBuffer / ANativeWindow_toSurface back the
 *   Camera2/ARCore capture path. No WebCamTexture, no UnityEngine.XR, no
 *   ARCore in the managed code. Unreachable.
 *
 *   ALooper_pollAll, socketpair, cosh/sinh/tanh, environ, tcflush are real and
 *   implemented.
 *
 * Every stub logs once through NOTSUP() so debug.log says plainly if one of
 * those "unreachable" assumptions turns out to be false.
 *
 * Wired into the combined table in imports.c as ndk_stub_functions /
 * ndk_stub_numfunctions -- the *_functions / *_numfunctions naming is the
 * loader core's convention, not a choice.
 */
#include <stdint.h>
#include <stdbool.h>
#include <stddef.h>
...
DynLibFunction ndk_stub_functions[] = {
...
size_t ndk_stub_numfunctions =
    sizeof(ndk_stub_functions) / sizeof(ndk_stub_functions[0]);
