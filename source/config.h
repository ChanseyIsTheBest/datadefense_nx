/* config.h -- DATA DEFENSE 1.3.11 Switch wrapper configuration
 * (Unity 6000.3.9f1, IL2CPP, arm64, landscape.)
 *
 * Header corrected: this claimed "Plants vs Zombies Fusion 3.6.1"
 ...
 * (see nx_patch_datadefense.h). Do not change unless you know the allocator math.
 * ======================================================================== */
...
// the libunity patch.
#define MMAP_ARENA_ALIGN    ((size_t)64 * 1024 * 1024)    // 64MB region granularity (libunity patched 256MB->64MB, nx_patch_datadefense.h). NOTE: 16MB was tried and CORRUPTED Unity's Dynamic Heap allocator at init (overlapping regions from the over-map/trim pattern -> null-prev free-list crash). 64MB is the known-good floor.
...
#define GAME_FOLDER  "datadefense_nx"   /* fallback only -- see nx_data_root.h */
 ...
 * output or debug.log. */
/* Bump this on every change you intend to boot. The whole point is that the
 * boot banner, the binary and this line must agree -- tools/verify_build.py
 * checks all three. A build that reports a stamp you do not recognise is a
 * build you are not testing. */
#define KB_SRC_REV   "dd-r43-onecursor"
...
#define GAME_PACKAGE "com.IIBlocks.DataDefense"
...
/* RETARGET: forced 0 -- hooks LeanTouch's GetTouchCount/GetTouch. Data Defense may not even use LeanTouch -- check before re-deriving.
 * Re-derive for Data Defense before turning this back on. */
#define KB_LEAN_INPUT_FROM_LOADER 0   /* round 159: the new input system is
 ...
 * injecting touches where its sprites are drawn (see source/dd_input.c). The
 ...
 * onto the game's own HUD (dd_input.c), a second cursor is not a second way to
 ...
 * so. The IAP module that did this was DELETED in this port -- Data Defense
 * is a paid app with no in-app purchases -- so nothing below is live. */
/* RETARGET: forced 0 -- Killer Bean's IAP entitlement methods. Data Defense is a paid app with no IAP, so this one is likely not needed at all.
 * Re-derive for Data Defense before turning this back on. */
#define KB_OFFLINE_PURCHASES 0
...
/* RETARGET: forced 0 -- Time.get_* icall thunk RVAs from Killer Bean's dump.cs.
 * Re-derive for Data Defense before turning this back on. */
/* Managed UnityEngine.Time hooks -- ON, and on this engine they are the ONLY
 * thing that makes time advance.
 *
 * The native path (KB_HAVE_TIME_FIX) installs cleanly and then never fires:
 * the log shows "[time] Update body resume = ..." but no "Update hook first
 * fire", i.e. TimeManager::Update is never called at all. Unity 6 also has no
 * GetTimeManager symbol (zero hits in 72,831), so nothing ever hands us the
 * TimeManager instance either. clayjam_nx hit exactly this and documented the
 * same signature -- frames advancing to 2 and then stopping while
 * PreloadManager and Loading.AsyncRead sit parked.
 *
 * So hook one level up, at the managed boundary: UnityEngine.Time's getters
 * are icall thunks in libil2cpp, and redirecting them makes C# see time move
 * regardless of what the native subsystem is doing.
 *
 * 7 of 9 getters resolved from the verified Il2CppInspector map; the other two
 * (smoothDeltaTime, timeSinceLevelLoad) were stripped by the managed linker
 * because this game never calls them, and the hook table skips a zero offset. */
#define KB_HAVE_TIME_HOOKS 1
/* RETARGET: forced 0 -- same family; a Killer Bean libunity offset.
 * Re-derive for Data Defense before turning this back on. */
#define KB_FORCE_SPLASH_FINISH 0  /* round 57: skip the stuck end-of-fade animation-event gate */
...
/* RETARGET: forced 0 -- tuned against Killer Bean's preload path.
 * Re-derive for Data Defense before turning this back on. */
#define KB_PRELOAD_NO_EARLY_EXIT 0
...
/* RETARGET: 1 -> 0. The packer exists because Zookeeper shipped ~4,200 loose
 * asset files and per-file open costs far more on Switch than on Android --
 * its first scene load took 20+ minutes unpacked. Data Defense ships NINE
 * files: data.unity3d, resources.resource, global-metadata.dat and six small
 * ones. There is nothing to pack. Leaving this on would spend the first boot
 * copying 64 MB into a pack, need 64 MB of extra SD space to do it, then
 * delete the originals -- all to save nine file opens. */
#define KB_ASSET_PACK 0
/* RETARGET: forced 0 -- patches GetShouldShowSplashScreen at a Killer Bean libunity offset.
 * Re-derive for Data Defense before turning this back on. */
#define KB_BYPASS_UNITY_SPLASH 0  /* round 58: GetShouldShowSplashScreen->0 (native Unity splash) */
 ...
 * See nx_patch_datadefense.h. Set to 0 for stock behaviour. */
...
/* RETARGET: forced 0 -- hooks Killer Bean's LevelMap_WhatsNew MonoBehaviour.
 * Re-derive for Data Defense before turning this back on. */
#define KB_WHATSNEW_HOOKS 0
#endif
...
/* RETARGET: forced 0 -- same.
 * Re-derive for Data Defense before turning this back on. */
#define KB_SKIP_WHATSNEW 0
#endif

#ifndef KB_HIDE_BLOCKING_PANELS
/* RETARGET: forced 0 -- closes Killer Bean's specific UI panels.
 * Re-derive for Data Defense before turning this back on. */
#define KB_HIDE_BLOCKING_PANELS 0
#endif

#ifndef KB_HOOK_CLOSE_WHATSNEW
/* RETARGET: forced 0 -- same.
 * Re-derive for Data Defense before turning this back on. */
#define KB_HOOK_CLOSE_WHATSNEW 0
#endif
...
/* GC stop-the-world bridge. Boehm stops the world by signalling every other
 * thread; on Switch those POSIX signals cannot be delivered, so without this
 * the collector waits forever on its ack semaphore and the game wedges.
 *
 * ON for Data Defense: all six globals were re-derived from THIS libil2cpp by
 * disassembling GC_suspend_all / GC_stop_world / GC_restart_all, located from
 * the "pthread_kill failed at suspend" abort string. See the GC section of
 * nx_patch_datadefense.h for the instruction-level evidence. */
#define KB_HAVE_GC_BRIDGE  1
 ...
 *                revisit nx_pointer, which maps the panel to render space
