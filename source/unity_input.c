...
/* Which of nativeInjectEvent's two instanceof tests we are answering.
 *
 * The engine classifies an InputEvent with exactly TWO tests, in a fixed order
 * (full disassembly of nativeInjectEvent, +0xaadda8..+0xaaedf8):
 *
 *   L_aadeb0  test(event, class @+0x1bedbd8)  -> match: KEY branch
 *   L_aadf20  test(event, class @+0x1bedd78)  -> match: MOTION branch
 *
 * It resolves those classes through jni::FindClass, and on this port that never
 * reaches our FindClass -- the boot resolves 15 classes and neither
 * android/view/KeyEvent nor MotionEvent is among them. So the class operand
 * arrives unidentifiable, and jni_fake's IsInstanceOf cannot answer by name.
 *
 * Its fallback was `return 1`, correct for the InputEvent BASE class and fatal
 * for an unknown one: the first test is the key test, so every touch matched it
 * and was read as a key. The getter census proves that is what happened --
 * getKeyCode, getMetaState, getRepeatCount, and never once getX or getY.
 *
 * Position is the only signal left, and it is a reliable one because the order
 * is fixed in the binary. The counter resets per injected event. */
static int s_instanceof_n = 0;
void input_begin_dispatch(void){ s_instanceof_n = 0; }
int  input_instanceof_next(void){ return s_instanceof_n++; }
...
/* The concrete Android class this object stands for, or NULL if it is not one
 * of ours. jni_fake.c's GetObjectClass hands back java/lang/Object for
 * everything it does not recognise, which leaves the ENGINE unable to tell a
 * MotionEvent from a KeyEvent -- and it guesses KeyEvent. The r34 log shows
 * exactly that: after the getKeyCode fix the engine reached getAction, then
 * asked for getRepeatCount (a KeyEvent-only method) and never once asked for
 * getX, getY or getPointerCount. It had committed to the wrong type. */
const char *unity_event_class(void *obj)
{
  const UEvent *e = obj;
  if (!e || e->tag != UI_TAG) return NULL;
  return (e->kind == KIND_MOTION) ? "android/view/MotionEvent"
...
uint64_t input_dispatch_int(void *recv, const void *id_, va_list va){ const struct FakeID *id = id_;
  ...
  if (e->kind==KIND_MOTION){
    /* getKeyCode() ON A MOTION EVENT MUST MATCH NOTHING.
     *
     * A real MotionEvent has no getKeyCode() at all, so Android never reaches
     * this. Our fake answers every method on every object, so the engine's
     * KeyEvent filter runs against touches too: it reads getKeyCode() and
     * compares the result to KEYCODE_VOLUME_DOWN, KEYCODE_VOLUME_UP,
     * KEYCODE_ZOOM_OUT and whatever else it screens for.
     *
     * Falling through to 0 made that comparison a coin-toss on jni_fake.c's
     * field table: every keycode constant it does NOT answer also reads back
     * as 0, so the touch matched whichever one was missing. Answering the
     * constants one at a time does not fix it -- there are ~300 of them, and
     * each boot simply found the next: VOLUME_DOWN, then VOLUME_UP, then
     * ZOOM_OUT.
     *
     * -1 ends the whole class. Every Android key code is >= 0
     * (KEYCODE_UNKNOWN is 0), so -1 equals no constant whether jni_fake
     * answers it or not, and the engine falls through to the motion path
     * regardless of which keys it happens to screen for. */
    if (has(m,"getKeyCode"))      return (uint64_t)(int64_t)-1;
