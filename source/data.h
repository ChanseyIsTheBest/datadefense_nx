/* data.h -- data layer: the AAsset NDK API over loose files, plus game-dir
 * path resolution.
 *
 * STILL LOAD-BEARING: jni_fake.c and cr3_stubs.c include it, and the AAsset
 * entry points it declares are implemented in ndk_stubs.c and imported by the
 * game. Only the description was stale.
 *
 * The original text described Chaos Rings 3's .mvgl archives and Play Asset
 * Delivery packs, none of which exist here -- Data Defense ships 9 loose files
 * under assets/bin/Data. The API is unchanged; the archive story is not ours.
 *
 * Original header, for lineage:
 *   Chaos Rings 3 data layer
 *
