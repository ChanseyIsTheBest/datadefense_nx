...
#include "util.h"
#include "so_util.h"
#include "nx_patch_datadefense.h"   /* KB_IL2CPP_LIVENESS_* frame map (round 129) */
 ...
 * from the caller's frame -- see the frame map in nx_patch_datadefense.h. */
/* The ConvertFromTo fault. Frame map and its cross-check: nx_patch_datadefense.h.
...
static void xd_databinding_triage(ThreadExceptionDump *ctx, u64 rel_pc) {
   ...
   * and dump what it was actually reading -- see nx_patch_datadefense.h for the
