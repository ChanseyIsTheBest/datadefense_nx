/* cr3_stubs.c -- no-op bodies for helpers that only the Chaos Rings (MVGL)
 * engine ever calls, kept so the shared headers link.
 *
 * AUDITED FOR DATA DEFENSE: still referenced (android_mark_main_finished,
 * data_dir, movie_play are named by jni_fake.c and the shared headers), so it
 * cannot simply be deleted -- but nothing it implements is reachable in a Unity
 * title. It exists to satisfy the link, not to do work.
 *
 * Original: no-op bodies for the Chaos-Rings-specific helpers that
