 ...
 * android_native_unity.c there is no ANativeActivity glue / android_main / AInputQueue
...
#define ALOOPER_POLL_CALLBACK (-2)   /* a registered fd callback was invoked */
#define ALOOPER_POLL_TIMEOUT  (-3)
#define ALOOPER_EVENT_INPUT   (1)    /* the fd is readable */
#define MAX_LOOPERS 16

/* Registered fd callbacks. Unity 6's NdkLooper is built on these: it creates a
 * pipe, registers the read end with ALooper_addFd(looper, fd, ident=-2,
 * events=1, callback, data), and posts a message by writing a byte. The looper
 * thread is expected to notice the fd is readable and run the callback, which
 * is what actually processes the message. */
#define MAX_LOOPER_FDS 8
struct LooperFd { int fd, ident, events; void *cb, *data; int used; };
struct ALooper { Mutex m; CondVar cv; int signaled; int refs; u32 owner; int used;
                 struct LooperFd fds[MAX_LOOPER_FDS]; };
...
/* CREATE ON DEMAND. This was create=0, inherited from the Killer Bean (2021.3)
 * and Deus Ex GO (2022.3) trees, where it is correct: those engines only ask
 * whether a looper exists.
 *
 * Unity 6 asks in order to USE it. InitializeUILooper() does
 *
 *     if (ALooper_forThread() == NULL) {
 *         log "Couldn't retrieve native ALooper for UI thread";
 *         return;                     // leaves the global NdkLooper NULL
 *     }
 *
 * and a later NdkLooper::CreateHandler() calls WaitForCreation() on that null
 * singleton -> `ldrb w8, [x0, #0xe8]` with x0 = 0. On a real device the UI
 * thread already owns a Java-created looper so the NULL branch never runs; we
 * have no Java, so we have to supply one.
 *
 * Handing back a fresh looper is consistent with the rest of this file rather
 * than a special case: ALooper_prepare() and ALooper_pollOnce() both already
 * pass create=1, so any thread that polls gets a looper whether it asked for
 * one or not. forThread() returning NULL was the odd one out.
 *
 * Credit: this is clayjam_nx's fix, and its comment names the internals
 * (InitializeUILooper, NdkLooper, WaitForCreation) that a symbol-less
 * disassembly of this binary cannot. Verified against our own crash: same
 * instruction, same +0xe8 field, same null global. */
...
/* Declared locally: this file deliberately does not include
 * android_native_unity.h (it defines these functions rather than consuming
 * them), and the fakefd layer lives in fakefd.c. Non-blocking query. */
int fakefd_readable(int fd);

/* Service every registered fd whose pipe has data. Returns the number of
 * callbacks run. Takes the looper lock itself and drops it around each
 * callback, because the callback reads the pipe and can re-enter the looper. */
typedef int (*ALooperCallback)(int fd, int events, void *data);
static int nx_looper_service_fds(struct ALooper *L) {
  if (!L) return 0;
  int ran = 0;
  mutexLock(&L->m);
  for (int i = 0; i < MAX_LOOPER_FDS; i++) {
    if (!L->fds[i].used || !L->fds[i].cb) continue;
    int fd = L->fds[i].fd;
    if (!fakefd_readable(fd)) continue;
    ALooperCallback cb = (ALooperCallback)L->fds[i].cb;
    void *data   = L->fds[i].data;
    int   events = L->fds[i].events ? L->fds[i].events : ALOOPER_EVENT_INPUT;
    mutexUnlock(&L->m);
    int keep = cb(fd, events, data);          /* 0 = unregister me */
    mutexLock(&L->m);
    ran++;
    if (!keep) L->fds[i].used = 0;
  }
  mutexUnlock(&L->m);
  return ran;
}
...
int ALooper_pollOnce(int timeoutMillis, int *outFd, int *outEvents, void **outData){
  ...
  /* Dispatch first: a message may already be pending from before we got here. */
  if (nx_looper_service_fds(L) > 0) return ALOOPER_POLL_CALLBACK;

  mutexLock(&L->m);
  if (!L->signaled){
    if (timeoutMillis==0){ mutexUnlock(&L->m); return ALOOPER_POLL_TIMEOUT; }
    /* Slice the wait so registered fds keep getting serviced even when nobody
     * calls ALooper_wake -- SendMessage only writes to the pipe, it does not
     * signal the condvar. A single slice is not enough: with timeoutMillis < 0
     * the caller asked to wait until something happens, and returning TIMEOUT
     * after one slice turns that into a spin. Loop until signaled or the
     * caller's timeout is spent. (Shape taken from hitmansniper_nx, whose port
     * runs; an earlier version here waited one 4ms slice and returned.) */
    u64 slice = 2000000ull;                    /* 2 ms */
    int remaining = timeoutMillis;             /* <0 means wait forever */
    for (;;){
      condvarWaitTimeout(&L->cv,&L->m,slice);
      if (L->signaled) break;
      mutexUnlock(&L->m);
      if (nx_looper_service_fds(L) > 0) return ALOOPER_POLL_CALLBACK;
      mutexLock(&L->m);
...
/* These were `return 1` stubs, with a comment saying Unity rarely uses them.
 * That was true of the Unity versions this tree came from. Unity 6 builds its
 * whole NdkLooper message path on them, so a no-op addFd means every message
 * posted to a looper is silently dropped -- which is how ChoreographerNDK's
 * constructor came to wait forever for a message it had already sent. */
int ALooper_addFd(ALooper *l,int fd,int ident,int events,void *cb,void *data){
  struct ALooper *L = l ? (struct ALooper *)l : looper_for(cur_tid(), 1);
  if (!L) return -1;
  mutexLock(&L->m);
  int slot = -1;
  for (int i = 0; i < MAX_LOOPER_FDS; i++)              /* replace same fd */
    if (L->fds[i].used && L->fds[i].fd == fd) { slot = i; break; }
  if (slot < 0)
    for (int i = 0; i < MAX_LOOPER_FDS; i++)
      if (!L->fds[i].used) { slot = i; break; }
  if (slot < 0) { mutexUnlock(&L->m); 
    debugPrintf("[looper] addFd: no free slot for fd %d\n", fd); return -1; }
  L->fds[slot].fd=fd; L->fds[slot].ident=ident; L->fds[slot].events=events;
  L->fds[slot].cb=cb; L->fds[slot].data=data; L->fds[slot].used=1;
  mutexUnlock(&L->m);
  debugPrintf("[looper] addFd fd=%d ident=%d events=0x%x cb=%p (slot %d)\n",
              fd, ident, events, cb, slot);
  return 1;
}
int ALooper_removeFd(ALooper *l,int fd){
  struct ALooper *L = l ? (struct ALooper *)l : looper_for(cur_tid(), 0);
  if (!L) return 0;
  mutexLock(&L->m);
  int n=0;
  for (int i = 0; i < MAX_LOOPER_FDS; i++)
    if (L->fds[i].used && L->fds[i].fd == fd) { L->fds[i].used = 0; n++; }
  mutexUnlock(&L->m);
  return n ? 1 : 0;
}
...
void android_native_input_init(void){
  ...
  /* One cursor now, not two. Pointer ids must stay SMALL: Unity maps an Android
   * pointer id into a fixed-size touch pool, and the module's old 100/101
   * defaults were silently dropped there -- the cursors drew and moved but
   * their taps never arrived. Real fingers take 0..7 and the cursor takes 8,
   * which is 9 pointers, inside UI_MAX_POINTERS, so unity_motionevent never
   * clamps. */
  c.max_touch_slots = 8;
  c.cursor_id       = 8;
...
static void emit(inject_fn inject, void *env, void *thiz, int action){
  ...
  input_begin_dispatch();   /* the two instanceof tests are per-event */
  const uint8_t ok = inject(env, thiz,
         unity_motionevent(action, g_live_n, g_live_id, g_live_x, g_live_y), 0);
  /* nativeInjectEvent's RETURN VALUE MEANS NOTHING HERE, and reading it as
   * CONSUMED/DROPPED sent this port chasing the wrong thing for two rounds.
   *
   * Disassembling it (+0xaadda8) shows w20 -- the value returned -- written in
   * exactly two places:
   *     +0xaaddb8  mov w20, w3     <- our own int argument, echoed back
   *     +0xaadde8  mov w20, wzr    <- only on the setjmp/exception path
   * Nothing else touches it. We pass 0, so it returns 0, so the log said
   * "DROPPED" for every event whether or not the engine handled it. The int is
   * not used for dispatch either; it is purely a pass-through.
   *
   * So the label is now the neutral `ret=N`. A 0 here means "we passed 0", not
   * "the engine refused it"; the only real signal is ret=0 after an exception,
   * which the [in.*] trace shows separately.
   *
...
void android_native_feed_hid(inject_fn inject, void *env, void *thiz){
  ...
  /* Input heartbeat -- ALWAYS ON, once every ~5s, and cheap.
   *
   * The existing [touch] trace goes through input_log_fn, which is budget
   * gated, so a log with no touch lines says nothing about whether input is
   * working: it could equally mean the trace was never armed. That ambiguity
   * cost a round -- "nativeInjectEvent never appears" looked like proof the
   * path was dead when it was only proof the tracer was off.
   *
   * This states the three facts that separate the possible causes:
   *   polled  -- did the pointer/touch layer produce anything at all?
   *   docked  -- which input path should be live (cursor vs touchscreen)?
   *   visible -- is the cursor active? It is hidden in handheld by default,
   *              and toggled with '+'.
   * Zero polled with cursor off in docked mode is a different bug from zero
   * polled with it on. */
  {
    static uint64_t last_ns; static unsigned polled_total, feeds;
    polled_total += (unsigned)n; feeds++;
    uint64_t now = armTicksToNs(armGetSystemTick());
    if (now - last_ns > 5000000000ull) {
      last_ns = now;
      debugPrintf("[input] %u feed(s), %u event(s) polled | docked=%d "
                  "cursor_visible=%d touch=%d\n",
                  feeds, polled_total,
                  /* nxdp_docked() is static inside nx_pointer.c and is a
                   * one-line wrapper over this; calling appletGetOperationMode
                   * directly keeps the pointer module's API as it is rather
                   * than widening it for a log line. */
                  appletGetOperationMode() == AppletOperationMode_Console,
                  nxp_cursor_visible(),
                  /* nx_pointer owns the touchscreen unconditionally in
                   * handheld -- there is no handle_touch switch to report. */
                  1);
      polled_total = 0; feeds = 0;
    }
  }
   ...
   * NxpEvent rather than injecting separately -- a bound button then travels
  ...
  { extern int dd_input_poll(NxpEvent *, int, int, int);
    const int room = (int)(sizeof ev / sizeof ev[0]) - n;
    if (room > 0) n += dd_input_poll(ev + n, room, screen_width, screen_height); }
    ...
    if (ev[i].phase == NXP_DOWN){
      ...
      { const int now_f = diag_current_frame();
        if (now_f - g_live_frame[idx] < KB_TOUCH_MIN_FRAMES) {
          /* Mark it and stop. nxp_poll reports a transition ONCE, so we cannot
...
/* ==========================================================================
 * Cursor overlay. nx_pointer draws both cursors (built-in arrow, or
