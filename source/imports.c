 ...
 * OpenSL ES (our shim), the libandroid NativeActivity API (android_native_unity.c),
...
static void *thread_trampoline(void *p) {
   ...
   *
   * This used to run after nx_spread_thread_core(), which left a window where
   * a thread exists and can block but is invisible to the watchdog. That is
   * not hypothetical: an intermittent boot hang had UnityMain parked in
   * PlatformThread::Create (+0xa8af0c pthread_create, then a cond_wait loop on
   * the new thread's "I am up" flag at [obj+0x20]) while `[fxown] STUCK ...
   * tid=6412` reported a thread stuck on a futex -- and 6412 was nowhere in
   * the watchdog roster, which listed three threads. A thread that can hang is
   * a thread that has to be visible, and TLS is the only thing that must come
   * first. */
  diag_thread_register(ts->entry, ts->is_main);
  { static unsigned n;
    if (n < 24) { n++;
      debugPrintf("[thr] started tid=%u entry=%p%s\n",
                  (unsigned)gettid_fake(), (void *)ts->entry,
                  ts->is_main ? "  (engine main)" : ""); } }
  /* DO NOT ENTER ENGINE CODE WHILE THE WORLD IS STOPPED.
   *
   * This thread is not in GC_threads yet -- Boehm registers a thread from
   * inside its own entry -- so a stop-the-world in progress neither paused it
   * nor is waiting for it. Running now means possibly taking a lock held by a
   * thread the collector HAS paused, and blocking until the world restarts.
   * Whoever is waiting on this thread blocks with it, and if that waiter is
   * the collector's own progress path, nothing restarts the world at all.
   *
   * That is the intermittent boot hang. UnityMain sat in
   * PlatformThread::Create's retry loop waiting for the new thread's "I am up"
   * flag at [obj+0x20], while the new thread sat on a futex with a suspended
   * owner. The race window is the gap between pthread_create returning and
   * this thread registering, which is why one boot worked and the next did not.
   *
   * Yielding here is safe in a way that running is not: the collector cannot
   * be waiting on a thread it does not know about, so nothing deadlocks on
   * this wait. The bound is generous because a GC pause on this port has been
   * measured at up to 869 ms, and giving up early would reintroduce exactly
   * the race being closed. */
  { unsigned spins = 0;
    while (nx_gc_world_stopped() && spins < 4000u) {   /* <= ~2 s */
    ...
    if (spins) {
      static unsigned n;
      if (n < 8) { n++;
        debugPrintf("[thr] tid=%u held %u.%01u ms at startup for a GC pause%s\n",
                    (unsigned)gettid_fake(), spins / 2u, (spins % 2u) * 5u,
                    (spins >= 4000u) ? " -- GAVE UP, world still stopped" : ""); }
    }
  }
...
// libandroid (android_native_unity.c)
...
DynLibFunction dynlib_functions[] = {
  ...
  // --- libandroid: NativeActivity API (android_native_unity.c) ---
...
extern DynLibFunction ndk_stub_functions[];  /* imports_ndk_stubs.c */
extern size_t ndk_stub_numfunctions;
extern DynLibFunction dd_extra_functions[];          /* imports_dd_extra.c */
extern size_t dd_extra_numfunctions;
...
static void build_combined(void) {
  ...
  g_combined_n = (int)dynlib_numfunctions + unity_dynlib_numfunctions
               + (int)firebase_extra_numfunctions + (int)ndk_stub_numfunctions
               + (int)dd_extra_numfunctions;
  ...
  memcpy(g_combined + off, ndk_stub_functions,
         ndk_stub_numfunctions * sizeof(DynLibFunction));
  off += ndk_stub_numfunctions;
  /* Data Defense (Unity 6) extras -- LAST, so it wins any name collision with
   * an older table. That matters for the Choreographer group: a stale no-op
   * must never shadow the real dispatcher. */
  memcpy(g_combined + off, dd_extra_functions,
         dd_extra_numfunctions * sizeof(DynLibFunction));
}
