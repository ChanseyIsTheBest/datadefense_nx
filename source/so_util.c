...
/* Is a module with this file name loaded? Backs dlopen_fake(): the engine
 * dlopen()s libunity.so / libmain.so by name to then dlsym() their exports,
 * and that must succeed even though we mapped them ourselves rather than
 * through a real loader. */
/* Basename of a path. so_load() stores mod->name as the FULL path it was
 * loaded from ("sdmc:/switch/<game>/libil2cpp.so"), while callers ask about a
 * library by whatever spelling they happen to have. Comparing the two
 * directly can never match. */
static const char *so_name_base(const char *p) {
  const char *s = strrchr(p, '/');
  return s ? s + 1 : p;
}

/* Is `name` one of the modules WE loaded? Compared by basename at both ends.
 *
 * This used to strcmp the caller's string against mod->name, i.e. a basename
 * against a full path, so it always returned 0. That made dlopen_fake refuse
 * our own libil2cpp.so, and on Unity 6 that is fatal rather than cosmetic:
 * UnityInitApplication does ClassLoader.findLibrary("il2cpp") then dlopen()s
 * whatever path comes back, and on failure puts up a modal
 * "Failed to load Il2CPP." dialog and blocks forever waiting for a dismissal
 * that cannot arrive. A silent black screen with a live main thread.
 *
 * Matching on basename is also the right semantics: the engine may ask for
 * "libil2cpp.so", or for the absolute path our own findLibrary handed it, and
 * both mean the module already mapped at mod->load_virtbase. */
int so_resolve_external_module(const char *name) {
  if (!name) return 0;
  const char *want = so_name_base(name);
  for (so_module *mod = so_list; mod; mod = mod->next)
    if (strcmp(so_name_base(mod->name), want) == 0) return 1;
  return 0;
}
