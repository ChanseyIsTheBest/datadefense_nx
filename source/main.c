/* main.c -- DATA DEFENSE Switch wrapper entry point.
 *
 * (Header corrected: this said "ZOOKEEPER DX" through five retargets. The code
 * is shared loader machinery, but a file that names the wrong game is how you
 * end up reasoning about the wrong game's behaviour.)
 *
...
#include "dd_assets.h"
#include "dd_touchhook.h"
#include "config.h"
#include "nx_patch_datadefense.h"  /* Fruit Ninja: recovered libunity patch + flags */
#include <dirent.h>
...
/* GAME_NAME never existed -- config.h defines GAME_FOLDER, GAME_PACKAGE and
 * GAME_HOME. An undefined identifier inside #pragma message makes the whole
 * pragma malformed, so GCC warned and DISCARDED it: the compile-time stamp
 * this exists to print has never actually printed. Which is unfortunate, given
 * it was added after a stale build cost a debugging round. */
#pragma message ("building " GAME_FOLDER " source rev " KB_SRC_REV \
                 " (DEBUG_LOG=" NX_STR(DEBUG_LOG) ")")
...
static void nx_clock_tick(void *tm) {
  ...
  /* m_StartupRef. Unity 6 stores it as a RationalTime {int64 ticks; u32 num;
   * u32 den} with seconds = ticks * den / num -- NOT as the double PvZ read at
   * tm+0xe8. Reading a double out of that struct would take the low 8 bytes
   * (the tick count) and reinterpret the bit pattern as an IEEE double, which
   * is not a wrong number so much as a meaningless one. */
  const char *sr = (const char *)tm + DD_TIME_STARTUPREF_OFF;
  int64_t  sr_ticks = *(volatile int64_t  *)(sr);
  uint32_t sr_num   = *(volatile uint32_t *)(sr + 8);
  uint32_t sr_den   = *(volatile uint32_t *)(sr + 12);
  double sref = sr_num ? (double)sr_ticks * (double)sr_den / (double)sr_num : 0.0;
  double newTime = sref + wall;
  { static int once = 0;
    if (!once) { once = 1;
      debugPrintf("[time] m_StartupRef @tm+0x%x = %lld ticks @ %u/%u = %.6fs "
                  "(newTime = ref + wall)\n",
                  (unsigned)DD_TIME_STARTUPREF_OFF, (long long)sr_ticks,
                  sr_num, sr_den, sref); } }
  ...
  { static uint64_t last_hb = 0;
    if (now - last_hb > 2000000000ull) {
      ...
      debugPrintf("[time] hb: Time.time=%llums wall=%llums frameCount=%llu "
                  ...
                  /* tm+0x160, not PvZ's 0xc8. 0xc8 is m_DeltaTime -- a
                   * RationalTime -- so this printed a delta-time tick count as
                   * a frame number, and it oscillated by +/-20 million between
                   * heartbeats. That looked exactly like a runaway render loop
                   * and cost a round chasing one. The offsets were derived and
                   * recorded as DD_TIME_FRAMECOUNT_OFF long before this line
                   * was left pointing at the old one. */
                  (unsigned long long)*(volatile uint64_t *)
                      ((char *)tm + DD_TIME_FRAMECOUNT_OFF),
...
static void nx_time_update_hook(void *tm) {
  ...
  /* Replicate the prologue the 16-byte stub overwrote. Offsets are DERIVED for
   * this build (nx_patch_datadefense.h); PvZ's were 0xc8 / 0xd0 / 0xf8 and the
   * layout moved:
   *     ldr  x8,  [x0, #0x160]   ldr  w9, [x0, #0x168]   ldrb w10, [x0, #0x1a8]
   *     add  x8, x8, #1          add  w9, w9, #1
   *     str  x8,  [x0, #0x160]   str  w9, [x0, #0x168]   cbz w10 -> continue */
  *(volatile uint64_t *)((char *)tm + DD_TIME_FRAMECOUNT_OFF) += 1;
  *(volatile uint32_t *)((char *)tm + DD_TIME_AUXCOUNT_OFF)   += 1;
  if (*(volatile uint8_t *)((char *)tm + DD_TIME_PAUSED_OFF) != 0) return;
  mutexLock(&g_clock_lock);
  nx_clock_tick(tm);
  mutexUnlock(&g_clock_lock);
}
static void nx_clock_thread(void *arg);   /* defined below */

void nx_start_clock_thread(void)
{
  static int started = 0;
  if (started) return;                 /* the time fix may also ask for it */
  started = 1;
  if (R_SUCCEEDED(threadCreate(&g_clock_thr, nx_clock_thread, NULL, NULL,
                               0x8000, 0x2C, -2)))
    threadStart(&g_clock_thr);
  else
    debugPrintf("[vsync] clock thread failed to start -- no vsync pulse\n");
}

/* Arm the native vsync pulse.
 *
 * SEPARATE FROM THE TIME FIX ON PURPOSE. These three globals used to be set
 * inside nx_install_time_fix(), which returns early unless the TimeManager
 * Update hook installs -- so with KB_HAVE_TIME_FIX off, the vsync counter was
 * never pulsed at all even though the triple was known.
 *
 * That coupling is wrong because the two solve different problems. The time
 * fix drives Unity's clock; this stops WaitVSync(long) blocking forever.
 * WaitVSync loops on `counter >= target` under a mutex/condvar, and on Android
 * the display driver increments the counter once per refresh. Nothing does
 * here, so a thread that calls it never returns. That is a hang, not a pacing
 * nicety, and it is worth arming on its own.
 *
 * ndk_choreographer.c does NOT cover this. It answers the NDK Choreographer
 * API Unity 6 asks for; this is Unity's own internal counter, a different
 * mechanism reached through a different path. */
void nx_arm_vsync_pulse(uintptr_t ub)
{
  ...
  debugPrintf("[vsync] triple armed: mutex=libunity+0x%x cond=+0x%x "
              "counter=+0x%x (counter is mutex+0x%x)\n",
              (unsigned)KB_VSYNC_MUTEX, (unsigned)KB_VSYNC_COND,
              (unsigned)KB_VSYNC_COUNTER,
              (unsigned)(KB_VSYNC_COUNTER - KB_VSYNC_MUTEX));
#else
  (void)ub;
  debugPrintf("[vsync] triple NOT armed (KB_HAVE_VSYNC_TRIPLE 0) -- any "
              "thread that calls WaitVSync will block forever\n");
#endif
}

static void nx_clock_thread(void *arg) {
  (void)arg;
  /* Bionic TLS before anything: this thread calls g_unity_update_body(), which
   * is libunity code, and libnx threads start with tpidr_el0 = 0. The tree
   * already did this here -- aaudio_shim.c and ndk_choreographer.c did not,
   * which is what produced the far=0x28 crash. */
...
static void nx_install_time_fix(void) {
   ...
   *
   * DATA DEFENSE (Unity 6000.3.9f1), TimeManager::Update @libunity+0x8d0638.
   * The field offsets below are this build's, not PvZ's -- the layout moved:
   *
   *   +0x00  ldr  x8,  [x0,#0x160]    frameCount    (PvZ 0xc8)
   *   +0x04  ldr  w9,  [x0,#0x168]    aux counter   (PvZ 0xd0)
   *   +0x08  ldrb w10, [x0,#0x1a8]    pause flag    (PvZ 0xf8)
   *   +0x0c  add  x8, x8, #1          <- last instruction the stub overwrites
   *   +0x10  add  w9, w9, #1
   *   +0x14  str  x8, [x0,#0x160]
   *   +0x18  str  w9, [x0,#0x168]
   *   +0x1c  cbz  w10, <body>
   *   +0x20  ret                      <- paused: bail out
   *   +0x24  sub  sp, sp, #0x90       <- THE REAL BODY STARTS HERE
   *
   ...
   * where the prologue happened to be that long. On 2021.3 it is 0x24, and so
   * it is here -- the runtime scan and KB_TIME_UPDATE_BODY agree at +0x8d065c.
   * Resuming at +0x10 instead
   * re-entered the sequence with x8/w9/w10 NEVER LOADED:
  ...
  /* GetTimeManager(). NOT a zero-safe cast: `ub + 0` is libunity's base, which
   * is non-NULL, so the clock thread's
   *     tm = g_get_time_manager ? g_get_time_manager() : g_tm;
   * would call the ELF header as a function. Guard on the offset, not on the
   * result.
   *
   * Unity 6 has no GetTimeManager symbol at all -- zero hits in 72,831 -- so it
   * is inlined or gone, and KB_TIME_GETMANAGER stays 0 here. That is fine: the
   * clock thread then uses g_tm, which nx_time_update_hook sets on its first
   * fire, and which is the more reliable source anyway. */
  /* VERIFY-FIRST, not just non-zero. A wrong address here is called as a
   * function, so check the thunk's first word (`mov w0,#7`) before trusting
   * it -- the same discipline the patch tables use. */
  g_get_time_manager = NULL;
  if (KB_TIME_GETMANAGER) {
    uint32_t gw = *(volatile uint32_t *)(ub + KB_TIME_GETMANAGER);
    if (gw == KB_WORD_GETTIMEMGR) {
      g_get_time_manager = (void *(*)(void))(ub + KB_TIME_GETMANAGER);
      debugPrintf("[time] GetTimeManager @libunity+0x%x verified (mov w0,#7)\n",
                  (unsigned)KB_TIME_GETMANAGER);
    } else {
      debugPrintf("[time] SKIP GetTimeManager @+0x%x: 0x%08x is not `mov w0,#7`"
                  " -- re-derive; the clock thread will fall back to g_tm\n",
                  (unsigned)KB_TIME_GETMANAGER, gw);
    }
  }
  uint32_t stub[4] = {
...
static void nx_create_asset_skeleton(void) {
  ...
  static const char *dirs[] = {
    ...
    /* NOT "il2cpp" followed by a slash: Unity stages into il2cpp_tmp/ and
     * renames into place,
...
static int nx_patch_libunity(uintptr_t ub) {
  /* PvZ Fusion 3.6.1 (Unity 2022.3.62f1c1): tables live in nx_patch_datadefense.h.
  ...
  /* Optional branch forces (only when you have located them; see nx_patch_datadefense.h). */
 ...
 * rationale: nx_patch_datadefense.h. Register facts this depends on, all read
 ...
 * See nx_patch_datadefense.h for the derivation. These two replacements are the
...
static void nx_boot_il2cpp_hacks(void) {
   ...
   * called from the IAP module -- which this port deletes outright,
   * Data Defense being a paid app with no in-app purchases. Not patched, so the real unlock/save code
  ...
  /* Round 156. See nx_patch_datadefense.h for the derivation and the evidence. */
...
int main(int argc, char *argv[]) {
  ...
  debugPrintf("[boot] === " GAME_FOLDER " start === rev " KB_SRC_REV
              "  built " __DATE__ " " __TIME__ "\n");
  debugPrintf("[boot] target: Data Defense 1.3.11, Unity 6000.3.9f1 IL2CPP, "
              "arm64, region64mb\n");
  ...
  /* Data Defense is a LANDSCAPE game -- report a landscape surface (W>H).
   * The Killer Bean tree this came from reported PORTRAIT (1080x1920) because
   * that game is portrait; inheriting it renders the whole game sideways into
   * a stretched buffer. Resolutions come from config.txt so both modes can
   * take either; 1080 in handheld is supersampled down to the panel. */
  {
    ...
    screen_width  = (res == 1080) ? 1920 : 1280;
    screen_height = res;
  }
  ...
  { char mp[768]; struct stat mst;
    ...
    /* No "you should repack" warning here any more. It was written for
     * Zookeeper DX, which ships ~4,200 loose resource files and genuinely does
     * take 20+ minutes to load a scene unpacked. Data Defense ships nine, and
     * the banner fired on every boot claiming otherwise -- directly
     * contradicting the [assets] block printed a few lines below it, which
     * counts the files for real. A warning that is wrong on this game teaches
     * you to skip the log. dd_assets_log() reports what is actually there. */
  }
  /* Find and check the game's files before anything tries to use them.
   * dd_assets_log() runs before the fatal check on purpose: when the layout is
   * wrong, debug.log is the only place the reason survives. */
  dd_assets_detect();
  dd_assets_log();
  dd_assets_fatal_if_incomplete();
  ...
  { /* Killer Bean ships NO .split parts (asset survey: 0 found), so this
    ...
    /* The files THIS game ships. The inherited list named
     * sharedassets1.resource and "unity default resources" at the Data root --
     * neither exists in Data Defense (its default resources live under
     * Data/Resources/). Every probe therefore failed and the loader ran a full
     * recursive readdir looking for .splitN parts on EVERY boot. */
...
#if KB_ASSET_PACK
  /* Build the pack on first boot, now that splits are joined -- but only if
   * the asset tree is actually the kind of tree the packer helps with. See
   * dd_assets_should_pack(): for Data Defense's nine files it is pure cost,
   * and it ends by deleting the loose tree the player copied over. */
  if (!_pack_mounted && dd_assets_should_pack()) {
  ...
  /* Arm Unity's native vsync counter. Independent of the TimeManager time fix
   * (see nx_arm_vsync_pulse) -- WaitVSync(long) blocks until this counter
   * advances, and on Android the display driver is what advances it. */
  nx_arm_vsync_pulse((uintptr_t)unity_mod.load_virtbase);
   ...
   * type fields). 0x2A1503E1 (mov w1,w21) -> 0x528002C1 (movz w1,#0x16). */
  {
    ...
    if (*(volatile uint32_t *)(ub + KB_FMOD_OUTPUT_SITE) == 0x2A1503E1u) {
      ...
      /* KB_FMOD_OUTPUT_SITE is 0 for Data Defense, so the read above lands on
       * libunity+0 and returns 0x464c457f -- the ELF magic. That is not a
       * failure and the old wording ("libunity differs from expected 62f1c1")
       * blamed a PvZ build this port has nothing to do with. */
      uint32_t w = *(volatile uint32_t *)(ub + KB_FMOD_OUTPUT_SITE);
      if (!KB_FMOD_OUTPUT_SITE)
        debugPrintf("[fmod] force-OpenSL not applied: NOT NEEDED on Unity 6. "
                    "AudioManager requests AUTODETECT(0), not AudioTrack(21); "
                    "FMOD probes AAudio (dlopen denied) then OpenSL (shimmed). "
                    "Set DD_HAVE_FMOD_FORCE_OPENSL if there is no audio.\n");
      else
        debugPrintf("[fmod] SKIP force-OpenSL: +0x%x = 0x%08x, not `mov w1,w21`"
                    " -- re-derive for this libunity\n",
                    (unsigned)KB_FMOD_OUTPUT_SITE, w);
    }
     ...
     * nx_patch_datadefense.h as KB_PACING_GUARD_WORD. */
    if (*(volatile uint32_t *)(ub + KB_PACING_GETTER) == KB_PACING_GUARD_WORD) {
      ...
      uint32_t w = *(volatile uint32_t *)(ub + KB_PACING_GETTER);
      if (!KB_PACING_GETTER)
        debugPrintf("[pace] Swappy-disable not applied: NOT NEEDED. Unity 6 "
                    "gates Swappy on dlopen(\"libswappywrapper\"), which is not "
                    "in dlopen_allow[] and returns NULL, so Swappy disables "
                    "itself. Look for the dlopen line below.\n");
      else
        debugPrintf("[pace] SKIP Swappy-disable: +0x%x = 0x%08x, not the "
                    "expected prologue -- re-derive for this libunity\n",
                    (unsigned)KB_PACING_GETTER, w);
    }
    ...
    if (KB_HAVE_FMOD_BUFFER_BYPASS &&
      ...
      if (!KB_FMOD_BUFFER_SITE)
        debugPrintf("[fmod] buffer-geometry bypass not applied: signature "
                    "absent in this FMOD build, as in the reference port.\n");
      else
  ...
  /* Give the boot thread a looper explicitly.
   *
   * NOT the fix -- ALooper_forThread() creating on demand is (see
   * android_native_unity.c). This is kept because it puts the fact in the boot
   * log: "Couldn't retrieve native ALooper for UI thread" sat in every log for
   * six revisions looking like a warning while being a null-pointer bomb, and
   * a positive statement that the UI thread has a looper is cheap insurance
   * against reading past it again.
   *
   * The first attempt at this bug was this call ALONE, on the reasoning that a
   * looper is per-thread state a thread should opt into. That reasoning was
   * wrong twice over: ALooper_prepare() and ALooper_pollOnce() in this tree
   * already create on demand for any caller, so forThread() was the
   * inconsistency rather than the safeguard; and preparing on one thread only
   * helps if that is the thread Unity asks from, at the moment it asks. */
  /* No local `extern` here: android_native_unity.h is already included above
   * and declares this as `ALooper *ALooper_prepare(int)`. Re-declaring it as
   * `void *` is a hard error, and would have been wrong even if it compiled --
   * a second declaration is a second thing to keep in step with the header. */
  { void *ui_looper = (void *)ALooper_prepare(0);
    debugPrintf("[boot] ALooper_prepare() for the UI thread -> %p%s\n",
                ui_looper,
                ui_looper ? "" : "  *** NULL: the looper table is full ***"); }
  ...
  {
/* Only declared when something below actually uses it. All three of these are
 * 0 for Data Defense, so without the guard `b` is an unused variable. The gate
 * list must match the #ifs below exactly -- I first wrote KB_HAVE_GC_BRIDGE
 * here, which is on but does NOT use b, so the warning would have persisted. */
#if KB_HAVE_IL2CPP_VM || KB_HAVE_ISINST_GUARD || KB_HAVE_LIVENESS_GUARD
    uintptr_t b = (uintptr_t)il2cpp_mod.load_virtbase;
#endif
#if KB_HAVE_IL2CPP_VM
    /* Fruit Ninja values, derived by disassembling THIS libil2cpp's
     * JNI_OnLoad @0x1588e1c -- see nx_patch_datadefense.h. Both targets are
...
#endif
#if KB_HAVE_IL2CPP_VM
    debugPrintf("[boot] il2cpp JavaVM global stored manually (vm=%p)\n", fake_vm);
#else
    /* This line used to print "il2cpp JavaVM global set" unconditionally while
     * the stores above sat inside #if KB_HAVE_IL2CPP_VM (0), so it announced
     * work it had not done. It is not needed here: unlike hitmansniper_nx --
     * which cannot call JNI_OnLoad, because its first act is an
     * __android_log_print through a GOT slot that is not bound yet, and so
     * replicates the two stores by hand -- our JNI_OnLoad runs and returns
     * JNI_VERSION_1_6, caching the VM itself. */
    debugPrintf("[boot] il2cpp cached the JavaVM via its own JNI_OnLoad "
                "(vm=%p); no manual store needed\n", fake_vm);
#endif
  ...
  /* Context type 0 = ActivityOrService (1 would be GameActivity), and a NULL
   * command line. See the long note on fn_initJni in unity_entrypoints.h --
   * Unity 6 added both parameters and the old 3-argument call passed register
   * garbage as the context type. */
  debugPrintf("[boot] calling initJni(ctx, type=0 ActivityOrService, cmdline=NULL)...\n");
  Unity_initJni(fake_env, fake_unityplayer_thiz, fake_context_obj, 0, NULL);
  debugPrintf("[boot] initJni returned; nativeRecreateGfxState...\n");
  /* Start the clock thread BEFORE graphics setup, not after.
   *
   * nx_arm_vsync_pulse() above only sets the three globals; the thread that
   * actually increments the counter is this one, and it used to start inside
   * nx_install_time_fix() -- which runs AFTER nativeRecreateGfxState,
   * nativeSendSurfaceChangedEvent and nativeResume.
   *
   * That is too late. AndroidGraphics::Startup runs inside the first
   * nativeRender and can reach WaitVSync(long), which blocks until the counter
   * advances. With the globals armed but nothing pulsing them, that is a hang
   * with no error: the main thread sits in cond_wait inside graphics init.
   *
   * Safe here rather than at nx_arm_vsync_pulse(): by this point libunity's
   * init arrays and initJni have run, so the pthread objects the pulse locks
   * are constructed. nx_start_clock_thread() is idempotent, so the later call
   * from nx_install_time_fix() is a no-op. */
  nx_start_clock_thread();

  debugPrintf("[boot] recreateGfxState(flag=0, surface=%p) [attaches the "
              "primary window: AttachSurface -> ANativeWindow_fromSurface -> "
              "AndroidGraphics::SetWindow]\n", fake_surface_obj);
  ...
  /* Input hooks go on AFTER il2cpp is loaded and initialised, and before the
   * first frame: they patch UnityEngine.Input's methods in libil2cpp, which
   * must exist and must not yet be running. */
  dd_touchhook_install(&il2cpp_mod);
...
#else
  /* Time fix OFF. Everything behind it IS derived for this build (see
   * KB_HAVE_TIME_FIX in nx_patch_datadefense.h) -- turning the gate off just
   * declines to overwrite TimeManager::Update.
   *
   * The clock thread still runs: it is what pulses the vsync counter, and that
   * is needed either way. Its TimeManager work no-ops while g_tm is NULL,
   * because only the Update hook sets g_tm. */
  nx_start_clock_thread();
#endif
  ...
  while (appletMainLoop() && !jni_quit_requested) {
    ...
    dd_touchhook_tick();     /* refresh the pointer the Input hooks report */
